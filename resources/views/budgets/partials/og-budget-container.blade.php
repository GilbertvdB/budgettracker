<div class="w-full bg-white dark:bg-gray-800 rounded-xl shadow-lg px-4 pt-2 pb-1 mb-4 {{ $budget->active == 0 ? 'opacity-50' : '' }}">
    ...
    <div class="flex justify-between">
        ...
        <form method="POST" action="{{ route('budgets.updateActiveStatus', $budget->id) }}" id="active-form-{{ $budget->id }}">
            ...
            <div class="flex items-center">
                ...
                <input type="radio" id="active_yes_{{ $budget->id }}" name="active" value="1" 
                    {{ $budget->active == 1 ? 'checked' : '' }} onchange="submitForm({{ $budget->id }})">
                <label for="active_yes_{{ $budget->id }}" class="ml-2">Active</label>

                <!-- Radio button for Inactive -->
                <input type="radio" id="active_no_{{ $budget->id }}" name="active" value="0" 
                    {{ $budget->active == 0 ? 'checked' : '' }} class="ml-4" onchange="submitForm({{ $budget->id }})">
                <label for="active_no_{{ $budget->id }}" class="ml-2">Inactive</label>
            </div>
        </form>
    </div>
    ...
    <div class="flex justify-between">
        ...
        <div>
            ...
            @method('PATCH')
                <!-- pin -->
                <button type="submit" form="pinned-form-{{ $budget->id }}">
                <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32" enable-background="new 0 0 32 32" xml:space="preserve" width="32px" height="32px" fill="currentColor" transform="matrix(-1, 0, 0, 1, 0, 0)rotate(0)">
